...
(function($){
	...
	var createGrips = function(t){
		...
		t.cg = t.find("col"); 						//at table can also contain a colgroup with col elements
		...
		th.each(function(i){						//iterate through the table column headers
			...
			var $ht = $('.header_table');
			$($ht.find('th')[i]).width(c.w);
			$($ht.find('th')[i+1]).width(c.w);
	...
	*/
	var syncCols = function(t,i,isOver){
		...
		var $ht = $('.header_table');
		$($ht.find('th')[i]).width(w + PX);
		$($ht.find('th')[i+1]).width(w2 + PX);
